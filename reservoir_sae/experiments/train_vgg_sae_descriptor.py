...
from reservoir_sae.utils.hf_data import infer_columns, load_hf_split, make_loader
...
def main() -> None:
    ...
    dataset = load_hf_split(args.dataset, args.split, cache_dir=args.cache_dir, token=args.hf_token)
    columns = infer_columns(dataset)
    dataset = dataset.select(range(min(args.max_samples, len(dataset))))
    loader = make_loader(dataset, transform, columns, batch_size=args.batch_size, num_workers=args.num_workers)

    pooler = VGGFeaturePooler().to(device).eval()
    features = []
    print("Collecting VGG pooled features...")
    with torch.no_grad():
        for step, (images, _labels, _meta) in enumerate(loader):
    ...
    print(f"Training VGG-SAE input_dim={input_dim} hidden_dim={hidden_dim} samples={len(normalized)}")
