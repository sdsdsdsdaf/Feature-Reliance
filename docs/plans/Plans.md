...
- 공통 모델: `vit_base_patch16_224` + SAE `outputs/reservoir_sae/vit_b_sae.pt`(block10·patch·`K=12288`, expansion 16). **⚠️ 교체 진행 중** — expansion 64(`K=49152`)·raw-space 손실로 재학습 중이며 grid 완료 후 확정한다(아래 Spec delta 2026-08-07). 그 전까지 모든 실험의 기준 계기는 이 옛 ckpt다.
...
- **2026-08-07 계기 교체 진행 (SAE 재학습)**: 실험 4A·T1.3·T1.4a가 반복해 각주로 달아둔 "계기가 무디다"(패치당 `L0 ≈ 497` / 768차원)를 해소하려고 SAE를 재학습 중이다. 사양: **expansion 64(`K=49152`)**, `recon_space="raw"`(denorm 공간 손실), token_budget 5.02억 토큰, ghost gradients. **아직 공통 모델은 옛 ckpt(`vit_b_sae.pt`, `K=12288`)이고 교체는 grid 완료 후 확정한다.**
  - **trial_0000(λ=1.8e-3) 실측**: `val_nmse` 0.0850 · `cosine` 0.9508 · `l0_raw` 330.6 · `active>0.2` 138.7 · `dead_latent_frac` 0.6476. `l0_raw`가 497 → 330.6으로 내려와 PatchSAE 운용점(148.56, `z>0` 기준)에 접근했으나 아직 **2.2배 조밀**하다.
  - **가장 큰 성과 — 재구성 바닥 붕괴**: 개입 진단에서 SAE를 경로에 끼우기만 한 `alpha=1.0` 대조군의 정확도 대가가 **20.3점 → 1.85점**(clean 0.8600 → 0.8415)으로 **11배** 줄었다. 총량 재구성 품질은 거의 안 움직였으므로(정규화 공간 `token_nmse` 0.130 vs 이전 FVU 0.1358) 원인은 품질 개선이 아니라 **손실 공간을 배포 경로(raw)에 맞춘 것**이다. 실험 2(T1.2)의 no-op 항등성 여유가 그만큼 커졌다.
  - **개입 부호 이상 해소**: cue latent 절제가 정확도를 **올리던** 현상(shape +4.34pt)이 사라지고 의도한 방향으로 복귀했다(shape −1.65pt, color −1.55pt, texture −0.50pt, α=1.0 대조군 기준). `js_divergence`가 matched-frequency-magnitude 대조군 대비 **10~100배**로 분리되고 random arm 3종은 바닥(0.0391)에 붙었다.
  - **dead latent 결정 확정**: 64.8%(`z>0.2`) / 61.9%(`z>0`)로 합의 기준 40%를 초과 → **TopK/BatchTopK 채택**. 다만 그 전에 ghost gradients(PatchSAE 이식)를 먼저 시도 중이다. 실측 A/B(400스텝, `window=0`): 미발화 latent 33,330 → 27,818, 대조군은 33,165로 평평 — **부활은 일어나나 L1이 다시 죽여 일부만 유지**된다.
  - **미반영(계기 교체 확정 시 해야 할 일)**: `contracts/phaseM.md`의 shape·후보 수(`[768,12288]`, 후보 4,738/12,288 등)와 `contracts/phase1.md`의 `L0 ≈ 497~800` 서술은 **옛 ckpt 기준**이라 재도출이 필요하다. T1.1의 FVU 게이트 반증 결론이 새 계기에서도 유지되는지도 재확인 대상이다(당시 각주가 "계기를 `L0` 수십 급으로 올린 뒤 재측정할 여지"를 남겨뒀다).
