...
\subsection{SAE Descriptor Extraction}
...
\subsection{Reservoir Maintenance}
...
\subsection{Comparison to ReservoirTTA}
...
We evaluate online accuracy, cumulative error, adaptation stability over recurrences, routing purity, and specialist reuse. We also report resource metrics: number of specialists, trainable parameters, memory cost, and inference time. Because SAE-latent routing adds encoding and similarity computation, the final analysis should include a cost-performance plot. The interpretability analysis will compare each specialist's prototype latents and identify which sparse features trigger routing to that specialist.

\section{Expected Results and Discussion Themes}

We expect SAE-latent routing to improve domains where the classifier's internal feature shift differs from simple style statistics. For example, two corruptions may share low-level style changes but activate different sparse semantic or local-shape latents; the proposed router should be able to separate them if these latent differences are stable. Conversely, if the domain stream is almost entirely explained by low-level style, StyleVec may remain competitive and cheaper.

The reservoir structure should improve stability compared with a single continuously adapted model, especially when domains recur. The main risk is efficiency: multiple specialists increase memory and the SAE adds an extra forward path. A second risk is interpretability dilution. Although SAE latents are more interpretable than dense features, the final behavior depends on both routing and specialist adaptation. The discussion will therefore separate routing interpretability, specialist behavior, and end-to-end accuracy.

Future experiments will test JumpReLU and Top-K SAE variants, lightweight latent pooling, prototype pruning, and hybrid routing that combines StyleVec with SAE latents. The most promising outcome is not just higher accuracy, but a reservoir whose specialists correspond to recurring sparse feature-reliance profiles.
